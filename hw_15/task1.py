...
class PhoneBook:
    ...
    output_phonebook:              Displays our contacts
    print_promt:                   Displays the menu our phonebook
    print_phonebook:               Outputs the existing list of contacts

    sort_phone_book_by_age:        Sorts the list of contacts
    add_new_contact:               Adding a new contact
    find_contact_by_name:          Display information by contact name
    find_contact_by_age:           Outputs information by contact age
    remove_contact_by_name:        Remove a contact by his name
    count_all_conacts:             Count the number of contacts
    avg_age_all_contacts:          Calculate the average age of all contacts
    increase_age_for_all_contacts: Increase the age of all contacts
    find_contact_by_operator:      Search for contacts by phone operator

    exit:           Close the program with the option to save changes
    save_to_file:   Save the changes to the existing phonebook
                    or we can save the changes to a new file
    load_from_file: Load the phonebook from another file
    '''
    ...
    def add_new_contact(self):
        self.new_contact = {}
        self.new_contact['surname'] = InputHandler.get_input_str(
            self, 'Введи фалимию: '
            ).capitalize()
        self.new_contact['name'] = InputHandler.get_input_str(
            self, 'Введи имя: '
            ).capitalize()
        self.new_contact['age'] = InputHandler.get_input_int(
            self, 'Введи возраст: '
            )
        self.new_contact['phone_number'] = InputHandler.get_input_int(
            self, 'Введи номер телефона: '
            )
        self.new_contact['phone_operator'] = InputHandler.get_input_str(
            self, 'Введи название оператора: '
            ).capitalize()
        self.phone_book.append(self.new_contact)

    def find_contact_by_name(self):
        self.input_find_name = InputHandler.get_input_str(
            self, 'Введи имя контакта, данные которого тебе нужны: '
            ).capitalize()
        self.find_name = True
    ...
    def find_contact_by_age(self):
        self.input_find_age = InputHandler.get_input_int(
            self, 'Введи возраст, для вывода контакта/ов: '
            )
        self.find_age = True
    ...
    def find_contact_by_operator(self):
        self.input_find_operator = InputHandler.get_input_str(
            self, 'Введи название оператора для вывода контактов: '
            ).capitalize()
    ...
    def remove_contact_by_name(self):
        self.user_input = InputHandler.get_input_str(
            self, 'Введи имя контакта которого хочешь удалить: '
            ).capitalize()
    ...
    def increase_age_for_all_contacts(self):
        self.user_input = InputHandler.get_input_int(
            self, 'Введи на сколько увеличить возраст контактов: '
            )
    ...
    def avg_age_all_contacts(self):
        self.ages = [contact['age'] for contact in self.phone_book]
        self.avg = sum(self.ages) / len(self.phone_book)

        print(f'Средний возраст всех людей в телефонной книге = {round(self.avg,2)}')

    def save_to_file(self):
        self.save_input = InputHandler.get_input_str(
            self, 'Перезаписать этот файл? (y/n)', ('y', 'n')
            ).lower()
    ...
    def load_from_file(self):
        if len(self.phone_book) > 0:
            self.answer = InputHandler.get_input_str(
                self, 'У вас есть существующие данные. Сохранить их перед загрузкой из файла? (y/n): ', ('y', 'n')
                ).lower()
            if self.answer == 'y':
                self.save_to_file()
    ...
    def exit(self):
        self.answer = InputHandler.get_input_str(
            self, 'У вас есть существующие данные. Сохранить их перед выходом из программы? (y/n): ', ('y', 'n')
            ).lower()
        if self.answer == 'y':
            self.save_to_file()
...
class User(PhoneBook):
    while PhoneBook.running:
        try:
            ...
            user_input = InputHandler.get_input_str(
                InputHandler, 'phonebook> ', '0123456789sla'
                )
